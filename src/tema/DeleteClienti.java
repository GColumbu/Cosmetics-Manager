...
public class DeleteClienti extends JPanel {
		...
		panelDelete.addMouseListener(new PanelButtonMouseAdapter(panelDelete) {
			...
			public void mouseClicked(MouseEvent e) {
				...
				try(Connection con = DriverManager.getConnection(connectionUrl); Statement stmt = con.createStatement();){
					String select = "select C.Nume AS Nume, C.Prenume as Prenume from Client C";
					...
					if(aux == 1) {
						String delete = "delete from Client where Nume = ('" + nume +"') and Prenume = ('" + prenume +"')";
						stmt.execute(delete);
